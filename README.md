...
3. **Environment-based API Protection**: Реализовано автоматическое скрытие документации Swagger UI/ReDoc и схемы OpenAPI в промышленной среде. При установке переменной APP_ENV=production доступ к /docs закрывается, что защищает сервис от автоматизированного сканирования (Reconnaissance). Для локального тестирования Swagger доступен по умолчанию.
4. **Reduced Attack Surface**: Финальный образ не содержит лишних утилит (curl, wget, git), а сетевой доступ к API кластера ограничен на уровне конфигурации пода.
5. **Type-Safe Validation (Pydantic)**: Для всех входных данных используются Pydantic-модели со строгой типизацией.
   Использование Literal и Enum для строковых полей (например, name_counter) полностью исключает возможность SSTI/XSS инъекций, так как любые некорректные символы отсекаются на этапе валидации запроса, не доходя до бизнес-логики или шаблонизатора.
Числовые поля ограничены валидными диапазонами (gt, lt), что предотвращает передачу аномальных значений.
...
### 📈 Grafana. Результаты стресс-тестирования памяти
На данном графике продемонстрирована работа планировщика и механизмов управления памятью в гетерогенном кластере:
*   **Node 1 (4GB, No Swap)**: Виден скачок потребления RAM (ступенька). Ресурс позволяет обрабатывать всплески нагрузки в оперативной памяти без потери производительности.
*   **Node 3 (1GB, Swap Enabled)**: При аналогичной нагрузке уровень RAM остается стабильным («полка»), в то время как график **Swap Usage** зеркально отображает рост нагрузки. Это демонстрирует механизм вытеснения страниц (paging) для предотвращения OOM-отказа ноды.
